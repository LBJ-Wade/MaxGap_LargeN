...
class Data_Input(object):
    ...
    def __init__(self, file_in, save_info=False, vocal=False):
...
#            cand_2 = np.vstack((quads[0], quads[2], quads[4], quads[6]))
            cand_2 = self.stack_quads(quads, indx_list=[0,2,4,6])
...
#            cand_3 = np.vstack((quads[4], quads[5], quads[6], quads[7]))
            cand_3 = self.stack_quads(quads, indx_list=[4,5,6,7])
...
#            cand_4 = np.vstack((quads[2], quads[3], quads[6], quads[7]))
            cand_4 = self.stack_quads(quads, indx_list=[2,3,6,7])
...
#            cand_5 = np.vstack((quads[1], quads[3], quads[5], quads[7]))
            cand_5 = self.stack_quads(quads, indx_list=[1,3,5,7])
    ...
    def stack_quads(self, quads, indx_list=[1,2,3,4]):
        idx_hold = 0
        for i in range(4):
            if len(quads[indx_list[i]]) > 0:
                ret_list = quads[indx_list[i]]
                idx_hold = i
                break
        for i in xrange(1+idx_hold, 4, 1):
            if len(quads[indx_list[i]]) > 0:
                ret_list = np.vstack((ret_list, quads[indx_list[i]]))
        return ret_list
